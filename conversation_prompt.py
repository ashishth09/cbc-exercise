...
def parse_conversation_response(json_data):
    ...
    out_dict = json_data
    texts = out_dict["output"]["text"]
    return {"text": "\n".join(texts), "context": out_dict["context"]}
...
def main_loop(conversation, workspace_id):
    ...
    response = conversation.message(message_input=None, workspace_id=workspace_id)
